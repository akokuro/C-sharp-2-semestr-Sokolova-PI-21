...
namespace AbstractRepairPlumbingServiceImplementDataBase
{
    public class ReportServiceDB : IReportService
    {
        ...
        public ReportServiceDB(AbstractRepairPlumbingDbContext context)
        {
            this.context = context;
        }
        public void SaveRepairPrice(ReportBindingModel model)
        {
            ...
            try
            {
               ...
               missing);
                ...
                var table = document.Tables.Add(rangeTable, repairs.Count, 2, ref
               missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = repairs[i].RepairName;
                    table.Cell(i + 1, 2).Range.Text = repairs[i].Price.ToString();
                }
        ...
        {
            return context.Storages.ToList().GroupJoin(context.StoragePlumbings.Include(r => r.Plumbing).ToList(), stock => stock,
            stockPlumbing => stockPlumbing.Storage,
            (stock, stockCompList) => new StoragesLoadViewModel
            {
                StorageName = stock.StorageName,
                TotalCount = stockCompList.Sum(r => r.Count),
                Plumbings = stockCompList.Select(r => new Tuple<string,
               int>(r.Plumbing.PlumbingName, r.Count))
            }).ToList();
        }
                        ...
                        if (elem.Plumbings.Count() > 0)
                        {
                           ...
                           Microsoft.Office.Interop.Excel.XlBorderWeight.xlThin;
                            ...
                            Microsoft.Office.Interop.Excel.XlColorIndex.xlColorIndexAutomatic,
                             1);
                            foreach (var listElem in elem.Plumbings)
                            {
        ...
        public void SaveClientOrders(ReportBindingModel model)
        {
           ...
           iTextSharp.text.Font.BOLD);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].RepairName, fontForCells));
                table.AddCell(cell);
